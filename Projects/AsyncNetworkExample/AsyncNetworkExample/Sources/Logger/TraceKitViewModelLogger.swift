...
public struct TraceKitViewModelLogger: ViewModelLogger {
    ...
    public func logPerformance(
        ...
        case .standard, .detailed:
            ...
            let thresholdStr = String(format: "%.3f", threshold)
            let message = "\(prefix) - \(operation): \(durationStr)s (threshold: \(thresholdStr)s)"
            TraceKit.log(
                level: finalLevel,
                message,
                category: viewModel,
    ...
    private func calculatePerformanceThreshold(
        operation: String,
        duration: TimeInterval
    ) -> (threshold: TimeInterval, shouldWarn: Bool) {
