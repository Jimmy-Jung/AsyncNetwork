...
struct PostRepositoryImplTests {
    ...
    func getPostById() async throws {
        ...
        // Then
        #expect(post.id == PostDTO.fixture().build().id)
    ...
    func testCreatePost() async throws {
        ...
        #expect(createdPost.title == PostDTO.fixture().build().title)
        #expect(createdPost.body == PostDTO.fixture().build().body)
    }
    ...
    func testUpdatePost() async throws {
        ...
        // @Response 매크로의 builder() 사용
        let updatedPost = Post(dto: PostDTO.fixture().build())
        ...
        #expect(result.id == PostDTO.fixture().build().id)
    }

    @Test("PostDTO mock 데이터가 도메인 모델로 변환되는지 확인")
    func postDTOMockConversion() throws {
        // Given - @Response 매크로의 mock() 사용
        let dto = PostDTO.random()
    ...
    func postDTOBuilderConsistency() {
        // Given - @Response 매크로의 builder() 사용
        let builder1 = PostDTO.fixture().build()
        let builder2 = PostDTO.fixture().build()
    ...
    func postDTOMockArray() throws {
        ...
        let dtos = PostDTO.randomArray(count: 5)
    ...
    func request(_ request: URLRequest) async throws -> HTTPResponse {
        ...
        if path.contains("/posts") && !path.contains("/comments") {
            ...
            if method == "GET" && path.components(separatedBy: "/").count > 2 {
                let dto = PostDTO.fixture().build()
                data = try JSONEncoder().encode(dto)
            }
            // GET /posts - 전체 Posts 조회
            else if method == "GET" {
                let dtos = PostDTO.randomArray(count: 2)
