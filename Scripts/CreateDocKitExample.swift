...
struct FileGenerator {
    ...
    func generateProjectSwift() -> String {
        ...
        for case .folder(let path) in config.sourcePathTypes {
        ...
        for case .tuistModule(let path, let moduleName) in config.sourcePathTypes {
...
struct CreateDocKitExampleScript {
    ...
    func runInteractive() {
        ...
        for (index, pathType) in sourcePathTypes.enumerated() {
            switch pathType {
            case .tuistModule(let path, let moduleName):
        ...
        for pathType in sourcePathTypes {
            let path: String
            switch pathType {
            case .tuistModule(let p, let moduleName):
                path = p
